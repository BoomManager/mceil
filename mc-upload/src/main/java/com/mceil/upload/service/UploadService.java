    ...
    public String uploadImage(MultipartFile file) {
        try {
            ...
            if(!prop.getAllowTypes().contains(contentType)){
                throw new McException(ExceptionEnum.INVALID_FILE_TYPE);
            }
...
//            //准备目标路径
//            File dest =new File("C:\\Users\\pc\\Desktop\\mceil\\upload",file.getOriginalFilename());
//            //保存图片到本地
...
//            String extension=file.getOriginalFilename().substring(file.getOriginalFilename().lastIndexOf(".")+1); 效率差
            ...
            //返回路径"http://image.mceil.com/" +
            return prop.getBaseUrl()+storePath.getFullPath();
        } catch (IOException e) {
            //上传失败
            log.error("[文件上传] 上传文件失败",e);
            throw new McException(ExceptionEnum.UPLOAD_FILE_ERROR);
        }
